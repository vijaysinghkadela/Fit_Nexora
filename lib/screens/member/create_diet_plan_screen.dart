...
class _CreateDietPlanScreenState extends ConsumerState<CreateDietPlanScreen>
  ...
  void _computeMacros() {
    ...
    _hydrationL =
        (_selectedSport?.key == 'bodybuilding' ||
  ...
  Future<void> _savePlan() async {
    ...
    try {
      ...
      final meals = _matchedTemplate?.meals ?? [];
      ...
      await db.createDietPlan({
        ...
        'goal': _selectedSport?.key ?? 'general_fitness',
        'target_calories': _targetCalories,
        'target_protein': _proteinG,
        'target_carbs': _carbsG,
        'target_fat': _fatG,
        'hydration_liters': _hydrationL,
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: _compactField('Height (cm)', _heightCtrl,
                              const TextInputType.numberWithOptions(decimal: true),
                              t),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: _compactField('Weight (kg)', _weightCtrl,
                              const TextInputType.numberWithOptions(decimal: true),
                              t),
                        ),
                      ],
        ...
        children: [
          ...
          GlassmorphicCard(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [t.brand.withOpacity(0.15), t.accent.withOpacity(0.08)],
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight,
                ),
          ...
          Row(
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () => _goToStep(1),
                  icon: Icon(Icons.edit_rounded, size: 16, color: t.textSecondary),
                  label: Text(
                    'Edit Macros',
                    style:
                        GoogleFonts.inter(color: t.textSecondary, fontSize: 14),
  ...
  Widget _buildTemplatesTab(dynamic t) {
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        final template = kDietTemplates[i];
        return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _infoChip(
                          '${template.targetCalories} kcal', Icons.local_fire_department_rounded, t),
                      _infoChip('P${template.targetProtein}',
                          Icons.egg_alt_rounded, t),
                      _infoChip(
                          'C${template.targetCarbs}', Icons.grain_rounded, t),
                      _infoChip('F${template.targetFat}', Icons.water_drop_outlined, t),
                    ],
                  ),
                  const SizedBox(height: 12),
                  SizedBox(
